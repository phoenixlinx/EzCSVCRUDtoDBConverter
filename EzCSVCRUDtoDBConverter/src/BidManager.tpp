        ...
        for (const auto& columnName : columnNames) {
            ...
            while (!validInput) {
                ...
                std::getline(std::cin, input);
                    ...
                    // Validate input type
                    if (columnType == typeid(std::string) || columnType == typeid(int) && StringConverter::isValidInt(input) || columnType == typeid(double) && StringConverter::isValidDouble(input)) {
                        rowData.push_back(input); // Append valid input to rowData
                        validInput = true; // Exit loop on successful input
                    }
                    else  {
                        std::cout << "Invalid input for column: " << columnName << std::endl;
                    }
